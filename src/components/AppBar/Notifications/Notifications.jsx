...
function Notifications() {
  ...
  // Biến state đơn giản để kiểm tra xem có thông báo mới hay không
  ...
  useEffect(() => {
    ...
    const onReceiveNewInvitation = (invitation) => {
      // Nếu như user đang đăng nhập hiện tại trong redux chính là invitee trong bản ghi invitation
  ...
  const updateBoardInvitation = (status, invitationId) => {
    dispatch(updateBoardInvitationAPI({ status, invitationId }))
      .then(res => {
        if (!res.error && res.payload.boardInvitation.status === BOARD_INVITATION_STATUS.ACCEPTED) {
